...
import { CartContext, useCart } from '../../services/cartService';
...
const Catalog = () => {
    ...
    const handleSendProduct = (product) => {
        console.log(product);
        navigate('/detalle', { state: { product } });
    };

    const displayedDocuments = products.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage);
                            ...
                            {displayedDocuments.map((producto) => (
                                <div key={producto.id} className="rounded-lg border border-gray-200 bg-card p-6 shadow-sm dark:border-gray-700 dark:bg-card_dark">
                                    <div className="h-56 w-full">
                                        <img className="mx-auto h-full cursor-pointer" src={producto.imageUrl} alt="" onClick={() => handleSendProduct(producto)} />

                                    </div>
                                    <div className="pt-6">
                                        <p onClick={() => handleSendProduct(producto)}
